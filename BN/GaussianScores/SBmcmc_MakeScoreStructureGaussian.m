...
% computed with Compute_Prior_Info.m
%
% INPUT, OPTIONAL
% - Nbest
...
Gauss_Score_complete_local(node,[]',data,T_0,T_m,v,alpha);
...
for parent1=1:Nnodes
    if parent1~=node
       nParent=nParent+1;
       LogBayesScores(nParent)=...%%%%%%%%%%%%%%CHANGE%%%%%%%%%%%%%%%%%%%%
       Gauss_Score_complete_local(node,parent1,data,T_0,T_m,v,alpha);
...
for parent1=1:Nnodes-1
    for parent2=parent1+1:Nnodes
        if parent1~=node & parent2~=node
           nParent=nParent+1;
           LogBayesScores(nParent)=...%%%%%%%%%%%%%%%%%%CHANGE%%%%%%%%%%%%%%%%%%%%%
           Gauss_Score_complete_local(node,[parent1,parent2]',data,T_0,T_m,v,alpha);
    ...
    for parent2=parent1+1:Nnodes-1
        for parent3=parent2+1:Nnodes
            ...
            if parent1~=node & parent2~=node & parent3~=node
              ...
              % parent3
               LogBayesScores(nParent)=...%%%%%%%%%%%%%%%%%%%%%CHANGE%%%%%%%%%%%%%%%%%%%%
               Gauss_Score_complete_local(node,[parent1,parent2,parent3]',data,T_0,T_m,v,alpha);
