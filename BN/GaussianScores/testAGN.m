...
for i=5:7
%    if (j==3 & ismember(s(i),[10 500 1000])) |(j==5 & ismember(s(i),[100 200])) | (j==6 & ismember(s(i),[50 100 200])) | (j==9 & s(i)==200)
...
%       fname1= sprintf('%s%d%s%d%s','H:\buf\NetRec\100g_10rec_scale-free_x_',j,'_',s(i),'_lowPC.mat');
      fname= sprintf('%s%s%s%d%s%d','H:\buf\NetRec\data\data_100gene\100g_10rec_',type,'_x_',j,'_',s(i),'measurements.txt');
      fname1= sprintf('%s%s%s%d%s%d','H:\buf\NetRec\data\bn\100g_10rec_',type,'_x_',j,'_',s(i),'_bn.mat');
...
cd('H:\buf\Software_Germany\GaussianScores');
%x=x(1:10,:);
